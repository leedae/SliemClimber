...
public class Character : MonoBehaviour 
{
	...
	public virtual void UpdateMovement() 
	{
		...
		// screen wrap
		if(thisTransform.position.x > 5.35f)
		{
			thisTransform.position = new Vector3(-5.35f,thisTransform.position.y, 0);
		}
		if(thisTransform.position.x < -5.35f)
		{
			thisTransform.position = new Vector3(5.35f,thisTransform.position.y, 0);
	...
	void UpdateRaycasts()
	{
			...
			|| Physics.Raycast(new Vector3(thisTransform.position.x+0.2f,thisTransform.position.y,thisTransform.position.z), Vector3.up, out hitInfo, rayDistUp+absVel2Y, groundMask))
		{
            ...
            GameObject goBlock = GameObject.Find(hitInfo.collider.name);
